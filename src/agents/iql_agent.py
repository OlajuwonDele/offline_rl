...
class IQLAgent(nn.Module):
    ...
    def update_q(
        ...
        loss = nn.functional.mse_loss(q, target.unsqueeze(0).expand_as(q).detach()).mean()
    ...
    def update_actor(
        ...
        v = self.value(observations)
        adv = (q - v).detach()
        weights = torch.exp(adv * self.alpha).clamp(max=M)
