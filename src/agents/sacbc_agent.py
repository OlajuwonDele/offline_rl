...
class SACBCAgent(nn.Module):
    ...
    def update_q(
        ...
        q = self.critic(observations, actions).mean(dim=0).squeeze(-1)
        target = rewards + self.discount * (1 - dones) * self.target_critic(next_observations, self.actor(next_observations).rsample()).mean(dim=0).squeeze(-1)
    ...
    def update_actor(
        ...
        q_loss = -self.critic(observations, actor_actions).mean(dim=0).mean()
